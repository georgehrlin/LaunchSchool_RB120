...
class Board
  ...
  def count_human_marker(squares)
    sqaures.collect(&:marker).count(TTTGame::HUMAN_MARKER)
  end

  def count_computer_marker(squares)
    sqaures.collect(&:marker).count(TTTGame::COMPUTER_MARKER)
  end
...
=end
      ...
      if @squares.select { |k, v| line.include?(k) }.values.all?(TTTGame::HUMAN_MARKER)
        return TTTGAME::HUMAN_MARKER
      elsif @squares.select { |k, v| line.include?(k) }.values.all?(TTTGame::COMPUTER_MARKER)
        return TTTGAME::COMPUTER_MARKER
      end
